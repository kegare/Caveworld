...
public class BlockPortalCaveworld extends BlockPortal implements IInventory
{
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		if (!world.isRemote && side >= 2)
		{
			...
			if (MCEconomyPlugin.enabled() && MCEconomyPlugin.SHOP >= 0 && (CaveworldAPI.isEntityInCaveworld(player) || CaveworldAPI.isEntityInCavern(player)))
			{
	...
	public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity)
	{
		if (!world.isRemote && entity.isEntityAlive())
		{
			...
			if (entity.timeUntilPortal <= 0)
			{
				...
				int dimNew = CaveworldAPI.isEntityInCaveworld(entity) ? CaveworldAPI.getLastDimension(entity) : CaveworldAPI.getDimension();

				if (dimOld == dimNew)
				{
					dimOld = 0;
					dimNew = CaveworldAPI.getDimension();
				}
				...
				Teleporter teleporter = new TeleporterCaveworld(worldNew, brick);
				...
				if (entity instanceof EntityPlayerMP)
				{
					...
					if (!player.isSneaking() && !player.isPotionActive(Potion.blindness))
					{
						...
						CaveworldAPI.setLastDimension(player, dimOld);
					}
				}
				else
				{
					...
					if (target != null)
					{
						...
						CaveworldAPI.setLastDimension(target, dimOld);
					}
