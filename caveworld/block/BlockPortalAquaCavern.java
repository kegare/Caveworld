...
public class BlockPortalAquaCavern extends BlockPortal implements IBlockPortal
{
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		if (side >= 2)
		{
			...
			if (world.isRemote)
			{
				Caveworld.proxy.displayPortalMenu(MenuType.AQUA_CAVERN_PORTAL, x, y, z);
			}
		}
	...
	public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity)
	{
		if (!world.isRemote && entity.isEntityAlive())
		{
			if (entity.timeUntilPortal <= 0)
			{
				...
				int dimNew = CaveworldAPI.isEntityInAquaCavern(entity) ? CaveworldAPI.getAquaCavernLastDimension(entity) : CaveworldAPI.getAquaCavernDimension();

				if (dimOld == dimNew)
				{
					dimOld = 0;
					dimNew = CaveworldAPI.getAquaCavernDimension();
				}
				...
				Teleporter teleporter = new TeleporterAquaCavern(worldNew, brick);
				...
				if (entity instanceof EntityPlayerMP)
				{
					...
					if (!player.isSneaking() && !player.isPotionActive(Potion.blindness))
					{
						...
						CaveworldAPI.setAquaCavernLastDimension(player, dimOld);
					}
				}
				else
				{
					...
					if (target != null)
					{
						...
						CaveworldAPI.setAquaCavernLastDimension(target, dimOld);
					}
	...
	private class Size
	{
		...
		protected int getPortalHeight()
		{
			...
			outside: for (portalHeight = 0; portalHeight < 21; ++portalHeight)
			{
				...
				for (i = 0; i < portalWidth; ++i)
				{
					...
					if (block == BlockPortalAquaCavern.this)
					{
						++portalBlockCount;
					}
		...
		{
			return block.getMaterial() == Material.air || block.getMaterial() == Material.water || block == BlockPortalAquaCavern.this;
		}
			...
			if (portalCoord != null && portalWidth >= 2 && portalWidth <= 21 && portalHeight >= 3 && portalHeight <= 21)
			{
				for (int i = 0; i < portalWidth; ++i)
				{
					...
					for (int j = 0; j < portalHeight; ++j)
					{
						if (portalDiffer)
						{
							if (worldObj.getBlock(x + 1, portalCoord.posY + j, z) == BlockPortalAquaCavern.this)
							{
								return false;
							}
							else if (worldObj.getBlock(x - 1, portalCoord.posY + j, z) == BlockPortalAquaCavern.this)
							{
								return false;
							}
						}
						...
						{
							if (worldObj.getBlock(x, portalCoord.posY + j, z + 1) == BlockPortalAquaCavern.this)
							{
								return false;
							}
							else if (worldObj.getBlock(x, portalCoord.posY + j, z - 1) == BlockPortalAquaCavern.this)
							{
								return false;
							}
						}
		...
		public void setPortalBlocks()
		{
			...
			for (int i = 0; i < portalWidth; ++i)
			{
				...
				for (int j = 0; j < portalHeight; ++j)
				{
					worldObj.setBlock(x, portalCoord.posY + j, z, BlockPortalAquaCavern.this, portalMetadata, 2);
				}
			}
		}
	}
