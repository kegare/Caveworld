...
public class BlockPortalCaveland extends BlockPortal implements IBlockPortal
{
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		if (side >= 2)
		{
			...
			if (world.isRemote)
			{
				Caveworld.proxy.displayPortalMenu(MenuType.CAVELAND_PORTAL, x, y, z);
			}
		}
	...
	public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity)
	{
		if (!world.isRemote && entity.isEntityAlive())
		{
			if (entity.timeUntilPortal <= 0)
			{
				...
				int dimNew = CaveworldAPI.isEntityInCaveland(entity) ? CaveworldAPI.getCavelandLastDimension(entity) : CaveworldAPI.getCavelandDimension();

				if (dimOld == dimNew)
				{
					dimOld = 0;
					dimNew = CaveworldAPI.getCavelandDimension();
				}
				...
				Teleporter teleporter = new TeleporterCaveland(worldNew, brick);
				...
				if (entity instanceof EntityPlayerMP)
				{
					...
					if (!player.isSneaking() && !player.isPotionActive(Potion.blindness))
					{
						...
						CaveworldAPI.setCavelandLastDimension(player, dimOld);
					}
				}
				else
				{
					...
					if (target != null)
					{
						...
						CaveworldAPI.setCavelandLastDimension(target, dimOld);
					}
	...
	public void randomDisplayTick(World world, int x, int y, int z, Random random)
	{
		...
		if (random.nextInt(3) == 0)
		{
			...
			world.spawnParticle("reddust", ptX, ptY, ptZ, 0.5D, 1.0D, 0.0D);
		}
	}
	...
	private class Size
	{
		...
		protected int getPortalHeight()
		{
			...
			outside: for (portalHeight = 0; portalHeight < 21; ++portalHeight)
			{
				...
				for (i = 0; i < portalWidth; ++i)
				{
					...
					if (block == BlockPortalCaveland.this)
					{
						++portalBlockCount;
					}
		...
		{
			return block.getMaterial() == Material.air || block == BlockPortalCaveland.this;
		}
			...
			if (portalCoord != null && portalWidth >= 2 && portalWidth <= 21 && portalHeight >= 3 && portalHeight <= 21)
			{
				for (int i = 0; i < portalWidth; ++i)
				{
					...
					for (int j = 0; j < portalHeight; ++j)
					{
						if (portalDiffer)
						{
							if (worldObj.getBlock(x + 1, portalCoord.posY + j, z) == BlockPortalCaveland.this)
							{
								return false;
							}
							else if (worldObj.getBlock(x - 1, portalCoord.posY + j, z) == BlockPortalCaveland.this)
							{
								return false;
							}
						}
						...
						{
							if (worldObj.getBlock(x, portalCoord.posY + j, z + 1) == BlockPortalCaveland.this)
							{
								return false;
							}
							else if (worldObj.getBlock(x, portalCoord.posY + j, z - 1) == BlockPortalCaveland.this)
							{
								return false;
							}
						}
		...
		public void setPortalBlocks()
		{
			...
			for (int i = 0; i < portalWidth; ++i)
			{
				...
				for (int j = 0; j < portalHeight; ++j)
				{
					worldObj.setBlock(x, portalCoord.posY + j, z, BlockPortalCaveland.this, portalMetadata, 2);
				}
			}
		}
	}
