...
public class GuiVeinsEntry extends GuiScreen implements SelectListener
{
			...
			switch (button.id)
			{
				...
				case 1:
					...
					else
					{
						...
						blockField.setText(GameData.getBlockRegistry().getNameForObject(veinList.selected.getBlock().getBlock()));
				...
				case 4:
					if (veinList.veins.remove(veinList.selected))
					{
						...
						veinList.selected = veinList.contents.get(--i, null);
					}
	...
	public void onSelected(Block block)
	{
		if (!editMode)
		{
			ICaveVein entry = new CaveVein(new BlockEntry(block, 0), 1, 1, 0, 255);
		...
		if (editMode)
		{
			...
			if (!isShiftKeyDown())
			{
				...
				else if (targetField.isFocused())
				{
					...
					mc.displayGuiScreen(new GuiSelectBlock(this, targetField));
				}
