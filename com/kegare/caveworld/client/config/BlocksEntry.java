...
public class BlocksEntry extends CaveCategoryEntry
{
	...
	protected List<IConfigElement> getConfigElements()
	{
		...
		{
			list.addAll(new BlockElement(getConfig().getCategory(category)).getChildElements());
		}

		return list;
	}

	private class BlockElement<T> extends ConfigElement<T>
	{
		...
		public BlockElement(ConfigCategory category)
		{
			super(category);
		...
		public BlockElement(Property prop)
		{
			super(prop);
			this.prop = prop;
		...
		public List<IConfigElement> getChildElements()
		{
			if (!isProperty)
			{
				...
				if (categoriesFirst)
				{
					while (ccI.hasNext())
					{
						ConfigElement temp = new BlockElement(ccI.next());
		...
		{
			return isProperty ? I18n.format("tile." + prop.getName() + ".name") : super.getName();
		}
		...
		public ConfigElement<?> getPropertyElement(Property prop)
		{
			...
			{
				case BOOLEAN:
					return new BlockElement<Boolean>(prop);
				case DOUBLE:
					return new BlockElement<Double>(prop);
				case INTEGER:
					return new BlockElement<Integer>(prop);
				default:
					return new BlockElement<String>(prop);
			}
		}
	}
}