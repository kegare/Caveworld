...
public class MapGenAquaCaves extends MapGenCavesCaveworld
{
	public MapGenAquaCaves()
	{
		super(false);
	}
		...
		for (boolean chance = random.nextInt(5) == 0; currentY < targetY; ++currentY)
		{
			...
			if (createFinalRoom || random.nextInt(4) != 0)
			{
				...
				if (blockX >= centerX - 16.0D - roomWidth * 2.0D && blockZ >= centerZ - 16.0D - roomWidth * 2.0D && blockX <= centerX + 16.0D + roomWidth * 2.0D && blockZ <= centerZ + 16.0D + roomWidth * 2.0D)
				{
					...
					int yLow = Math.max(MathHelper.floor_double(blockY - roomHeight) - 1, 2);
	...
	protected void func_151538_a(World world, int x, int z, int chunkX, int chunkZ, Block[] blocks)
	{
		...
		for (int i = 0; i < chance; ++i)
		{
			...
			for (int j = 0; j < count; ++j)
			{
				...
				func_151541_a(rand.nextLong(), chunkX, chunkZ, blocks, blockX, blockY, blockZ, scale * 2.75F, leftRightRadian, upDownRadian, 0, 0, 1.2D);
			}
		}
	}
